        (function() {
            ...
            function showPaymentModal(type) {
                return new Promise((resolve, reject) => {
                    ...
                    // Item 6: the "watch an ad instead" option is removed from generation —
                    // stars are now the only payment method here.
            ...
            function buildMessageEl(m) {
                ...
                if (m.loading) {
                    ...
                    if (m.loadingKind === "media") {
                        ...
                        thinkingImg.src = "img/thinking.gif";
                        thinkingImg.alt = "Thinking…";
                        thinkingImg.className = "chat-thinking-gif";
                        thinkingImg.addEventListener("error", () => {
                            // Fallback to the classic three-dot indicator if the GIF isn't uploaded yet.
                            loadWrap.innerHTML = `
                    ...
                    `;
                        ...
                        thinkingLabel.setAttribute("data-i18n", "thinking_label");
                        thinkingLabel.textContent = (() => {
                            try { return (localStorage.getItem('freebies_lang') || 'en') === 'ru' ? 'Думаю…' : 'Thinking…'; } catch (e) { return 'Thinking…'; }
                        })();
                        loadWrap.appendChild(thinkingImg);
                        loadWrap.appendChild(thinkingLabel);
                    }
            ...
            async function callGemmaRU(prompt, imageBase64 = null, mimeType = null) {
                ...
                const res = await fetch('/api/api?action=openrouter-ru', {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
            ...
            async function callChatAI(prompt, imageBase64 = null, mimeType = null) {
                let lastError = null;
                // Item 3: if the interface language is set to Russian, always use the
                // Gemma model (regardless of the user's detected geo-location).
                const uiLang = (() => { try { return localStorage.getItem('freebies_lang') || 'en'; } catch (e) { return 'en'; } })();
                const isRu = uiLang === 'ru';

                // Russian interface language: Gemma (OPENROUTER_BASE_KEY) is primary, with the
                // existing Gemini / Laguna chain kept intact as fallback if it's unavailable.
                if (isRu) {
                    try {
                ...
                try {
                    if (intent.type === "text") {
                        ...
                        const [reply] = await Promise.all([
                            callChatAI(text, imageBase64, mimeType),
                            minDelay,
                        ]);
            ...
            const I18N = {
                en: {
                    ...
                    hero_sub: "Powerful AI tools. No limits. Just watch ads and create.",
                ...
                ru: {
                    ...
                    hero_sub: "Мощные AI-инструменты. Без ограничений. Смотри рекламу и создавай.",
