        (function() {
            ...
            // Отдельная модель + отдельный ключ (OPENROUTER_BASE_KEY на сервере),
            // используется как ОСНОВНАЯ модель только для пользователей из России.
            const OPENROUTER_RU_MODEL = "google/gemma-4-26b-a4b-it:free";
            ...
            /* ============ AUTH HELPERS (provider-agnostic: Clerk or VK) ============ */
            function isAuthed() {
                if (window.AppAuth) return window.AppAuth.isLoggedIn();
                return !!window.Clerk?.user;
            }
            function openLoginPrompt() {
                if (window.AppAuth && window.AppAuth.isRussia()) {
            ...
            async function syncStarsFromClerk() {
                ...
                loadDailyStateFromSource(user.publicMetadata || user.unsafeMetadata);
                if (ensureDailyStarsFresh()) {
                    await persistDailyStarsToClerk();
                }
                updateDailyStarsUI();
                updateStars();
            }

            // VK / OK / Mail users (Russia): stars live entirely in localStorage,
            // as requested — there is no server-side sync for this provider.
            function syncStarsFromVK() {
                const vkUser = window.AppAuth?.vk?.getUser?.();
                if (!vkUser) {
                    state.stars = 0;
                    state.starsLoaded = false;
                    updateStars();
                ...
                const existing = window.AppAuth.vk.readStars();
                if (existing !== null) {
                    state.stars = existing;
                } else {
                    state.stars = window.AppAuth.vk.NEW_ACCOUNT_BONUS_STARS || NEW_ACCOUNT_BONUS_STARS;
                    window.AppAuth.vk.writeStars(state.stars);
                }
                state.starsLoaded = true;
                const dailyRaw = window.AppAuth.vk.readDaily();
                loadDailyStateFromSource(dailyRaw);
                if (ensureDailyStarsFresh()) {
                    persistDailyStarsToVK();
                }
                updateDailyStarsUI();
                updateStars();
            }

            function persistDailyStarsToVK() {
                if (!window.AppAuth?.vk?.getUser?.()) return;
                window.AppAuth.vk.writeDaily({
            ...
            async function persistStarsToClerk() {
                // VK users (Russia): persist purely to localStorage.
                const vkUser = window.AppAuth?.vk?.getUser?.();
                if (vkUser) {
                    window.AppAuth.vk.writeStars(state.stars);
                    return;
            ...
            async function persistDailyStarsToClerk() {
                // VK users (Russia): persist purely to localStorage.
                const vkUser = window.AppAuth?.vk?.getUser?.();
                if (vkUser) {
                    persistDailyStarsToVK();
                ...
                try {
                    const meta = user.publicMetadata || {};
                    await user.update({ publicMetadata: { ...meta, ...payload } });
                } catch (e) {
                    try {
                        const meta2 = user.unsafeMetadata || {};
                        await user.update({ unsafeMetadata: { ...meta2, ...payload } });
                    } catch (e2) {
                        console.warn('Could not save daily stars/free uses:', e2);
                    }
                }
            }
            ...
            function openDonateModal(starsCount, priceAmount) {
                const userId = (window.AppAuth ? window.AppAuth.getUserId() : window.Clerk?.user?.id) || null;
            ...
            if (donateCopyBtn) {
                donateCopyBtn.addEventListener('click', () => {
                    const id = window.AppAuth ? window.AppAuth.getUserId() : window.Clerk?.user?.id;
                    if (!id) return;
            ...
            async function updateUsername(newName) {
                const provider = window.AppAuth ? window.AppAuth.getProvider() : (window.Clerk?.user ? 'clerk' : null);
                if (!provider) {
                ...
                try {
                    if (provider === 'clerk') {
                        ...
                        if (vkUser) {
                            vkUser.name = newName;
                            try { localStorage.setItem('freebies_vk_user', JSON.stringify(vkUser)); } catch (e) {}
                        }
                    }
            ...
            /* ============ TABS ============ */
            const ALL_TABS = ["home", "chat", "gen-image", "unlimited-gen", "gen-video", "coder", "photos", "videos", "star-shop", "history", "profile", "contacts"];
            ...
            // Основная модель для российских пользователей: google/gemma через
            // отдельный ключ OPENROUTER_BASE_KEY (не путать с обычным OpenRouter-ключом).
            async function callGemmaRU(prompt, imageBase64 = null, mimeType = null) {
                const messages = [
                    { role: "system", content: getSystemPrompt() },
                    { role: "user", content: prompt },
                ];
                const res = await fetch('/api/openrouter-ru', {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        model: OPENROUTER_RU_MODEL,
                        messages,
                        max_tokens: 800,
                    }),
                });
                if (!res.ok) {
                    const errText = await res.text().catch(() => "");
                    throw new Error(`Gemma RU error (${res.status}): ${errText.slice(0, 200)}`);
                }
                const data = await res.json();
                let content = data?.choices?.[0]?.message?.content;
                if (!content) throw new Error("Empty response from Gemma RU.");
                return cleanText(content);
            }

            async function callChatAI(prompt, imageBase64 = null, mimeType = null) {
                let lastError = null;
                const isRu = window.AppAuth ? window.AppAuth.isRussia() : false;

                // Russia: Gemma (OPENROUTER_BASE_KEY) is primary, with the existing
                // Gemini / Laguna chain kept intact as fallback if it's unavailable.
                if (isRu) {
                    try {
                        return await callGemmaRU(prompt, imageBase64, mimeType);
                    } catch (err) {
                        console.warn('Gemma RU failed, falling back to standard chain:', err);
                        lastError = err;
                    }
                }
            ...
            window.syncStarsFromVK = syncStarsFromVK;
        ...
        function updateUI() {
            // Provider-agnostic: works whether the active session is Clerk
            // (rest of the world) or VK ID (Russia, via window.AppAuth).
            const isRu = window.AppAuth ? window.AppAuth.isRussia() : false;
            const provider = window.AppAuth ? window.AppAuth.getProvider() : null;
            const user = provider === 'clerk' ? window.Clerk?.user : (provider === 'vk' ? window.AppAuth.vk.getUser() : null);

            // Only auto-open Clerk's sign-in for non-Russian visitors — Russian
            // visitors are greeted by the VK ID gate instead (see authGate controller).
            if (!user && !autoLoginShown && clerkReady && !isRu) {
                autoLoginShown = true;
                setTimeout(() => {
            ...
            if (user) {
                const fullName = window.AppAuth ? window.AppAuth.getUserName() : (user.fullName || user.username || 'User');
                const email = window.AppAuth ? window.AppAuth.getUserEmail() : (user.primaryEmailAddress?.emailAddress || 'email@gmail.com');
                const userId = window.AppAuth ? window.AppAuth.getUserId() : user.id;
                ...
                if (accountInfoBtn) {
                    accountInfoBtn.textContent = provider === 'vk' ? `👤 ${fullName}` : `👤 ${email}`;
                ...
                if (userId !== lastSeenUserId) {
                    ...
                    if (provider === 'clerk' && typeof window.syncStarsFromClerk === 'function') {
                        window.syncStarsFromClerk();
                    } else if (provider === 'vk' && typeof window.syncStarsFromVK === 'function') {
                        window.syncStarsFromVK();
                    }
                }
                ...
                if (!registrationBonusHandled && window.state && !window.state.registrationBonusClaimed) {
                    ...
                    if (typeof window.claimRegistrationBonus === 'function') {
                        ...
                        <div style="background:#d4edda;padding:12px;border-radius:12px;margin-bottom:12px;border:1px solid #28a745;">
                          ...
                          +${window.NEW_ACCOUNT_BONUS_STARS || 110} ⭐ stars
                        </div>
                ...
                if (accountInfoBtn) {
                    ...
                    accountInfoBtn.onclick = () => {
                        if (isRu) { const gate = document.getElementById('authGate'); if (gate) gate.classList.remove('hidden'); }
                ...
                if (lastSeenUserId !== null) {
                    ...
                    if (provider === 'clerk' && typeof window.syncStarsFromClerk === 'function') {
                        window.syncStarsFromClerk();
                    } else if (typeof window.syncStarsFromVK === 'function') {
                        window.syncStarsFromVK();
                    }
                }
            }
        ...
        // Keep the UI in sync when VK ID logs a user in/out (region-aware auth gate).
        if (window.AppAuth) {
